...
Join in TA Data
```{r}
pHData <- left_join(pHData, TA, by = "sample_ID")
```

adding in TA for KB 2026/02/28
```{r}
pHData <- pHData %>% replace_na(list(TA = 2200))
```
...
pHSlope <- pHcalib %>% 
           ...
           drop_na(temp_pool, temp_field, salinity_field, TA) %>% # drop any rows that have NAs in the specified columns
  
            mutate(pH = pH(Ex=mV,Etris=mVTris,S=salinity_pool,T=temp_field)) %>% # calculate pH of the samples using the pH seacarb function
  
  mutate(pH_insitu = pHinsi(pH = pH, ALK = TA, Tinsi = temp_pool, Tlab = temp_field, S = salinity_field)) %>%
  
            select(date, sample_ID, pH, pH_insitu) # selects just these columns 
