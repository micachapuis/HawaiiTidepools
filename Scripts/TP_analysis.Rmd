...
kb_wind <- read_csv(here("Data", "Climate", "KB_0551_20250101_20260511.csv"))
sb_wind <- read_csv(here("Data", "Climate", "SB_0531_20250101_20260511.csv"))
```
...
```{r}
data %>% ggplot(aes(x = pH, y = pH_insitu)) + geom_point() + geom_abline(intercept=0,slope=1)
```

```{r}
data %>% ggplot(aes(x = temp_pool, y = pH_insitu)) + geom_point() + facet_wrap(~pool_number)
```
...
```{r}
data <- data %>% filter(!date %in% c("2025-06-08"))
```
...
Append test data 
```{r}
testdata <- testdata %>% rename("pH_insitu" = "pH") # not actually in situ bc didn't have pool salinity so can't calculate, but joining it in for the purposes of presenting until I get new data

testdata <- testdata %>% filter(!date %in% "2025-02-16") # filter out first day of test sampling
testdata <- testdata %>% filter(site %in% "Sandy Beach") # take Diamond Head out

data <- bind_rows(data, testdata) 
```

Remove Pools 1 and 17 (suspected groundwater)
```{r}
data <- data %>% filter(!(pool_number %in% c(1, 17)))
```

```{r}
data <- data %>% drop_na(salinity_field)
```
...
CO2 <- carb(flag = 8, # pH and ALK given
            data$pH_insitu, # first variable given
...
select(pH_insitu = pH, DIC, pCO2 = pCO2insitu, CO2, CO3, HCO3, OmegaCalcite, OmegaAragonite) 
```
...
OLD -- Calculate Delta TA and Delta DIC between two consecutive timepoints
```{r}
#deltas <- data %>% 
...
deltas <- data %>%
  ...
  select(date, site, pool_number, substrate, pool_surface_area_m2, mean_wind_speed, pool_volume, time_point, sample_time, temp_pool, pH_insitu, TA_norm, DIC_norm, FCO2, dissolved_oxygen, OmegaAragonite) %>%
  ...
  pivot_wider(
    ...
    values_from = c(pH_insitu, TA_norm, DIC_norm, temp_pool, sample_time, pool_volume, FCO2, dissolved_oxygen, OmegaAragonite),
    names_glue = "{.value}{time_point}") %>%
  
  mutate(
    delta_pH1 = pH_insitu2 - pH_insitu1, # change in pH between two sampling points
    delta_pH2 = pH_insitu3 - pH_insitu2,
    mean_pH1 = mean(pH_insitu2, pH_insitu1, na.rm = TRUE), 
    mean_pH2 = mean(pH_insitu3, pH_insitu2, na.rm = TRUE),
   ...
   pivot_longer(
      ...
      names_to = c(".value", "interval"), # interval will be 1 or 2 (1 is from T1 to T2 and 2 is from T2 to T3), .value takes the name of whichever value we get the delta for
      names_pattern = "(.*?)([12])$") %>%  # this renames all the columns to remove the 1s or 2s
  select(!(pH_insitu1:OmegaAragonite3)) %>%
...
data %>% ggplot(aes(x = time_shift, y = temp_pool, color = substrate)) +
  ...
  scale_x_continuous(
    ...
    expand = c(0, 0),
    breaks = seq(0, 24 * 60 * 60 - 1, by = 6 * 3600),  # every 6 hours (avoid 86400 endpoint)
    labels = function(x) {
      substr(as.character(hms::as_hms(((x + start) %% (24 * 60 * 60)))), 1, 5) # drops the seconds from x axis label
    }
  ) 
...
### pH
```{r}
data %>% filter(site %in% "Kaihalulu Beach") %>% ggplot(aes(x = sample_time, y = pH_insitu, color = pool_number)) +
...
```
```{r}
data %>% filter(site %in% "Sandy Beach") %>% ggplot(aes(x = sample_time, y = pH_insitu, color = pool_number)) +
    facet_wrap(~site, scales = "free_x") + geom_line(linewidth = 0.8) + geom_point() + theme_minimal() + labs(title = "pH (tris)")  + theme(axis.text.x = element_text(angle = 30))
```
...
```
```{r}
data %>% ggplot(aes(x = sample_time, y = pH_insitu)) +
    facet_wrap(~substrate) + geom_smooth() + geom_point() + theme_minimal() + labs(title = "pH")  + theme(axis.text.x = element_text(angle = 30))
```


```{r}
data %>% ggplot(aes(x = sample_time, y = pH_insitu, color = site)) +
    facet_wrap(~substrate) + geom_smooth() + geom_point() + theme_minimal() + labs(title = "pH")  + theme(axis.text.x = element_text(angle = 30))
```


```{r}
data %>% ggplot(aes(x = sample_time, y = pH_insitu, color = substrate)) +
    facet_wrap(~substrate, 
               labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) + 
  geom_smooth(method = "gam") + #, se=FALSE) + 
  geom_point() + theme_bw() +
...
```{r}
data %>%
  filter(!site %in% "Diamond Head") %>%
  ggplot(aes(x = time_shift, y = pH_insitu, color = substrate)) +
    facet_wrap(~substrate, 
               labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) + 
  geom_smooth(method = "gam", alpha = 0.3, color = "gray70", se = FALSE) + #method = "lm", se=FALSE) + 
  geom_point() + 
  theme_bw() +
  guides(color = "none") +
  scale_color_manual(values = c("#1A85FF", "#D41159", "#11D48C")) +
  theme(#axis.text.x = element_text(angle = 25),
  ...
  labs(x = "Sample Time", 
   ...
   scale_x_continuous(
    limits = c(0, 24 * 60 * 60 - 1),      # full day: 06:30 -> 06:29 next day
    expand = c(0, 0),
    breaks = seq(0, 24 * 60 * 60 - 1, by = 6 * 3600),  # break every 6 hours (avoid 86400 endpoint)
    #labels = function(x) format(hms::as_hms((x + start) %% (24 * 60 * 60)), "%H:%M")
    labels = function(x) {
      substr(as.character(hms::as_hms(((x + start) %% (24 * 60 * 60)))), 1, 5) # drops the seconds from x axis label
    }
...
ggsave(here("Output", "pH_over_time.png"), width = 9, height = 6)
```

#### pH pool vs ocean
```{r}
data %>% 
  filter(!pool_number %in% "ocean") %>%
  ggplot(aes(x = time_shift, y = pH_insitu)) +
  #  facet_wrap(~substrate, 
  #             labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) + 
  geom_smooth(method = "gam", color = "gray30") + #method = "lm", se=FALSE) + 
  ...
  theme(#axis.text.x = element_text(angle = 25),
        ...
        strip.text = element_text(size = 14)) +
  geom_vline(xintercept = line_shift, linetype = "dashed", color = "gray40") + # vertical line at sunset
  labs(x = "Sample Time", color = "Substrate") +
   scale_x_continuous(
    limits = c(0, 24 * 60 * 60 - 1),      # full day: 06:30 -> 06:29 next day
    expand = c(0, 0),
    breaks = seq(0, 24 * 60 * 60 - 1, by = 6 * 3600),  # break every 6 hours (avoid 86400 endpoint)
...
ggsave(here("Output", "pH_pool_vs_ocean.png"), width = 8, height = 6)
```
...
```{r}
# temp normalize
data %>% ggplot(aes(x = time_shift, y = (pH_insitu*temp_field)/(mean(data$temp_field)), color = substrate)) +
    facet_wrap(~substrate, 
               labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) + 
...
```{r}
# temp + sal normalize
data %>% ggplot(aes(x = time_shift, y = (((pH_insitu*temp_field)/(mean(data$temp_field)))*salinity_field)/mean(data$salinity_field), color = substrate)) +
    facet_wrap(~substrate, 
               labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) + 
  geom_smooth(method = "gam") + #method = "lm", se=FALSE) + 
  geom_point() + 
...
```{r}
data %>% ggplot(aes(x = time_shift, y = pH_insitu, color = substrate)) +
    facet_wrap(~substrate, 
               labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) + 
  geom_smooth(method = "gam") + #method = "lm", se=FALSE) + 
  geom_point() + 
...
data %>% 
  #filter(!site %in% "Diamond Head") %>%
  ggplot(aes(x = time_shift, y = TA_norm, color = substrate)) +
    facet_wrap(~substrate, 
               labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) + 
  geom_smooth(method = "gam") + #, se=FALSE) + 
  geom_point() + theme_bw() +
...
data %>% 
  #filter(!site %in% "Diamond Head") %>%
  #filter(!pool_number %in% c(2,3,6,15)) %>%
  ggplot(aes(x = time_shift, y = TA, color = substrate)) +
    facet_wrap(~substrate, 
               labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) + 
  geom_smooth(method = "gam") + #method = "lm", se=FALSE) + 
  geom_point() + 
...
data %>% ggplot(aes(x = time_shift, y = dissolved_oxygen, color = substrate)) +
  ...
  scale_x_continuous(
    limits = c(0, 24 * 60 * 60 - 1),      # full day: 06:30 -> 06:29 next day
    expand = c(0, 0),
    breaks = seq(0, 24 * 60 * 60 - 1, by = 6 * 3600),  # every 3 hours (avoid 86400 endpoint)
        labels = function(x) {
      substr(as.character(hms::as_hms(((x + start) %% (24 * 60 * 60)))), 1, 5) # drops the seconds from x axis label
    }
  )
...
####Delta Temp vs Delta pH, NEP, NEC
```{r}
deltas %>% filter(!site %in% "Diamond Head") %>% ggplot(aes(x = delta_temp, y = delta_pH, color = substrate)) + geom_point() + facet_wrap(~substrate)
```

```{r}
deltas %>% filter(!site %in% "Diamond Head") %>% ggplot(aes(x = delta_temp, y = NEP, color = substrate)) + geom_point() + facet_wrap(~substrate)
```

```{r}
deltas %>% filter(!site %in% "Diamond Head") %>% ggplot(aes(x = delta_temp, y = NEC, color = substrate)) + geom_point() + facet_wrap(~substrate)
```

#### Delta pH and TA by substrate
```{r}
...
deltas %>% 
  ...
  #             labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone"))) +
  theme_bw() + 
  labs(x = expression(paste(Delta, " pH ", " h"^-1 )), 
       y = expression(paste(Delta, " Dissolved Oxygen", " h"^-1 )),
       color = "Substrate") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
  theme(strip.text = element_text(size = 16),
...
ggsave(here("Output", "delta_pH_vs_O2.png"), width = 8, height = 6)
```
...
deltas %>% 
  ...
  filter(!pool_number %in% c("ocean")) %>% 
  ggplot(aes(x = delta_pH_norm, y = NEC, color = substrate, shape = substrate)) + 
  ...
  theme(axis.text = element_text(size = 18),
        ...
        plot.margin = margin(0.5, 0.5, 0, 0.2, "cm")) + 
  

  labs(y = expression(paste("NCC (mmol m"^-2," h"^-1,")")),
       x = expression(paste(Delta, " pH")) ) + #, 
  #     color = "Substrate", shape = "Substrate") +
 #scale_color_manual(labels = c("Basalt", "Limestone"), values = c("#1A85FF", "#D41159")) +
  scale_color_manual(values = c("#1A85FF", "#D41159")) #+
  #guides(color = "none", shape = "none")
  # c("#F8766D", "#00BA38") --> red and green
  #scale_shape(labels = c("Basalt", "Limestone")) +
...
deltas %>% 
  ...
  ggplot(aes(x = NEP, y = delta_pH_norm, color = substrate)) + 
  ...
  labs(x = expression(paste("Net Community Production (mmol m"^-2," h"^-1,")")),
       y = expression(paste(Delta, " pH", " h"^-1 )), 
       color = "Substrate") +
  scale_color_manual(labels = c("Basalt", "Limestone"), values = c("#1A85FF", "#D41159")) #+
  #scale_color_manual(values = c("#1A85FF", "#D41159")) +
  #guides(color = "none")
...
deltas %>% 
  ...
  labs(y = expression(paste("NEP (mmol m"^-2," h"^-1,")")),
       x = expression(paste(Delta, " Temperature")),
       color = "Substrate") +
  scale_color_manual(labels = c("Basalt", "Limestone"), values = c("#F8766D", "#00BA38"))
```
...
deltas %>% 
  filter(!pool_number == "ocean") %>%
  ggplot(aes(x = NEP, y = NEC, color = substrate)) + 
  geom_point(size = 3, alpha = 0.6) + 
  geom_smooth(method = "lm") +
  #facet_wrap(~substrate) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  ...
  labs(x = expression(paste("Net Community Production (mmol m"^-2," h"^-1,")")),
       y = expression(paste("Net Community Calcification (mmol m"^-2," h"^-1,")")) ) + #, 
  #     color = "Substrate", shape = "Substrate") +
  #scale_color_manual(labels = c("Basalt", "Limestone"), values = c("#1A85FF", "#D41159")) +
  scale_color_manual(values = c("#1A85FF", "#D41159")) +
  guides(color = "none")
...
```{r}
data %>% filter(!site %in% "Diamond Head") %>% 
  ggplot(aes(x = pH_insitu, y = OmegaAragonite, color = substrate)) + 
  geom_point() + 
  coord_trans(y = "log")+
  facet_wrap(~substrate,
             labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) +
...
```
```{r}
data %>% filter(site %in% "Kaihalulu Beach") %>% 
  ggplot(aes(x = pH_insitu, y = OmegaAragonite, color = producer_pcover)) + 
  geom_point() + 
  coord_trans(y = "log")+
  facet_wrap(~pool_number) +
  labs(y = expression(paste(Omega ['Aragonite']))) +
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 16)) +
  theme_bw()
...
```{r}
arag_ph_mod <- lm(OmegaAragonite ~ pH_insitu*substrate, data) 
Anova(arag_ph_mod)
summary(arag_ph_mod)
```
...
```{r}
data %>% #filter(!substrate %in% "ocean") %>% 
  ggplot(aes(x = pH_insitu, y = OmegaCalcite, color = substrate)) + 
  geom_point() + 
  coord_trans(y = "log")+
  facet_wrap(~substrate,
             labeller = as_labeller(c(basalt = "Basalt", limestone = "Limestone", ocean = "Ocean"))) +
...
```{r}
calc_ph_mod <- lm(OmegaCalcite ~ pH_insitu*substrate, data) 
Anova(calc_ph_mod)
summary(calc_ph_mod)
```
...
```{r}
#pH_mod <- lm(((pH_insitu*temp_field)/(mean(data$temp_field))) ~ sample_time*substrate, data = (data %>% filter(!substrate == "ocean")))
pH_mod <- lm(pH_insitu ~ sample_time*substrate, data = (data %>% filter(!substrate == "ocean")))
summary(pH_mod)
anova(pH_mod)
```
...
Fit regression for both substrate types combined
```{r}
TA_mod <- lm(TA ~ sample_time, data = (data %>% filter(!substrate == "ocean") %>% filter(!time_point == 2.5)))
summary(TA_mod)
```

```{r}
#commenting out bc error
data_resid_TA <- augment(TA_mod, (data %>% filter(!substrate == "ocean") %>% filter(!time_point == 2.5)))
```
...
```{r}
mean_pH_comp <- lm(mean_pH ~ total_producer_pcover, deltas)
summary(mean_pH_comp)
```

```{r}
mean_pH_comp_benthic <- lm(mean_pH ~ producer_pcover, deltas)
summary(mean_pH_comp_benthic)
```

```{r}
mean_pH_comp_mobile <- lm(mean_pH ~ inv_producer_pcover, deltas)
summary(mean_pH_comp_mobile)
```
...
```{r}
max_pH_data <- data %>% 
  group_by(substrate, pool_number) %>% 
  summarise(max_pH = max(pH_insitu)) %>%
          #se_pH = sd(pH_insitu, na.rm = TRUE) / sqrt(n())) %>%
  #distinct(substrate, .keep_all = TRUE) %>% 
  select(substrate, max_pH) %>%
  filter(!substrate %in% "ocean")

t.test(max_pH ~ substrate, max_pH_data)
```



```{r}
pH_NEP <- lm(delta_pH_norm ~ NEP*substrate, deltas)
summary(pH_NEP)
anova(pH_NEP)

coef(pH_NEP)

means_pHNEP <- emmeans(pH_NEP, ~ substrate) # gives average delta pH at each substrate
print(means_pHNEP)

slopes_pHNEP <- emtrends(pH_NEP, ~ "substrate", var = "NEP")
print(slopes_pHNEP)
pairs(slopes_pHNEP)

emm_intercepts_pHNEP <- emmeans(pH_NEP, "substrate", at = list(NEP = 0))
print(emm_intercepts_pHNEP)
pairs(emm_intercepts_pHNEP)
```

```{r}
NEC_pH <- lm(NEC ~ delta_pH_norm*substrate, deltas)
summary(NEC_pH)
Anova(NEC_pH)

means_NECpH <- emmeans(NEC_pH, ~ substrate) # gives average NEC at each substrate
print(means_NECpH)
pairs(means_NECpH)


slopes_NECpH <- emtrends(NEC_pH, ~ "substrate", var = "delta_pH_norm")
print(slopes_NECpH)
pairs(slopes_NECpH)

emm_intercepts_NECpH <- emmeans(NEC_pH, "substrate", at = list(delta_pH = 0))
print(emm_intercepts_NECpH)
pairs(emm_intercepts_NECpH)
```

```{r}
NEC_NEP <- lm(NEC ~ NEP*substrate, deltas)
summary(NEC_NEP)
Anova(NEC_NEP)


slopes_NECNEP <- emtrends(NEC_NEP, ~ "substrate", var = "NEP")
print(slopes_NECNEP)
pairs(slopes_NECNEP)

emm_intercepts_NECNEP <- emmeans(NEC_NEP, "substrate", at = list(NEP = 0))
print(emm_intercepts_NECNEP)
pairs(emm_intercepts_NECNEP)
